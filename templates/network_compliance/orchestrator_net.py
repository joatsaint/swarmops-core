"""
SwarmOps — Network Compliance Agent
Milestone 3 Domain Template

Drop this into any folder with a SCOPE.md and a watch_folder/.
Point your firewall/IDS log export at watch_folder\telemetry.log.
Run: python .\\orchestrator_net.py

Network events monitored:
  firewall_violation   Blocked outbound/inbound on known-bad port (23, 445ext, 3389ext)
  port_scan            Single source probing 5+ ports in < 60 seconds
  geo_anomaly          Traffic to/from country not in baseline allow-list
  unusual_outbound     First-seen external IP for this host
  dns_anomaly          DNS traffic on port other than 53
  auth_storm           10+ failed auth attempts in 5 min on SSH/RDP/VPN

Receipt vocabulary (Nate B Jones — Open Engine pattern):
  SWARM CLAIMED      Engine started processing an event
  SWARM DONE         Analysis complete, packet written to dispatched_drafts/
  SWARM BLOCKED      Event out of scope — not in SCOPE.md allow-list
  SWARM HUMAN HOLD   Confidence below threshold — waiting for human decision
"""
import os
import sys
import time
...
# Anchored to the script's own location so the agent works
...
NET_SEVERITY_GUIDE = """
Network Event Severity Reference:
  firewall_violation (known-bad port 23/Telnet, 445/SMB-ext, 3389/RDP-ext):
      HIGH — these ports should never reach external destinations
  port_scan (5+ ports from single source in < 60s):
      HIGH — active reconnaissance pattern
  geo_anomaly (traffic to/from unexpected country):
      HIGH — out-of-baseline destination; verify before dismissing
  unusual_outbound (first-seen external IP for this host):
      MEDIUM — could be legitimate software update or data exfil; context needed
  dns_anomaly (DNS on port != 53):
      MEDIUM — DNS tunneling indicator; correlate with other events
  auth_storm (10+ failed auth in 5 min on SSH/RDP/VPN):
      HIGH — brute-force pattern; may indicate compromised credential list
"""
...
# ==========================================
# RECEIPT VOCABULARY
# Standardized audit tokens — turn audit.log into a parseable
# compliance record, not just a narrative. grep-able after 30 days.
# ==========================================
_event_counter = 0
...
def _next_event_id() -> str:
    ...
    _event_counter += 1
    return f"evt-{datetime.now().strftime('%Y%m%d%H%M%S')}-{_event_counter:04d}"

def write_receipt(event_id: str, token: str, detail: str = "") -> None:
    ts = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
    line = f"{token} {event_id}"
    if detail:
        line += f" {detail}"
    logging.info(line)
...
def verify_controls():
    ...
    # Control 1: SCOPE.md — log approved actions at startup (scope expansion audit)
    if not os.path.exists("SCOPE.md"):
        ...
        if not actions:
            ...
            log_audit(f"Approved actions at startup: {', '.join(actions)}")
    ...
    if errors:
        ...
        for e in errors:
            ...
            print(f"  [X] {e}")
        print("=" * 60)
        sys.exit("Fix the above issues before running SwarmOps Network Agent.")
...
def write_to_approval_boundary(content_summary, full_output):
    ...
    boundary_text = (
        "==================================================\n"
        "SWARMOPS NETWORK AGENT — HUMAN REVIEW REQUIRED\n"
...
# ==========================================
# NETWORK COMPLIANCE LIVE PIPELINE
# ==========================================
def run_net_pipeline():
    log_audit("SwarmOps Network Compliance Agent: Initializing...")
    verify_controls()
    verify_and_parse_scope()

    if not os.path.exists(LOG_FILE):
        with open(LOG_FILE, "w", encoding="utf-8") as f:
            f.write(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} INFO [System] Network telemetry stream initialized.\n")

    print(f"\n--- SwarmOps Network Compliance Agent ---")
    print(f"[*] Confidence threshold: {CONFIDENCE_THRESHOLD}/100")
    print(f"[*] Monitoring: {LOG_FILE}")
    print(f"[*] Events: firewall_violation | port_scan | geo_anomaly | unusual_outbound | dns_anomaly | auth_storm")
    print(f"[*] Receipt vocabulary: SWARM CLAIMED | SWARM DONE | SWARM BLOCKED | SWARM HUMAN HOLD")
    print(f"[*] Awaiting network events...\n")
    ...
    try:
            ...
            if os.path.exists(LOG_FILE):
                ...
                if current_size > last_position:
                    ...
                    for line in lines:
                        ...
                        log_audit(f"Network event received: '{raw_event}'")

                        # Scope check — SWARM BLOCKED if action not authorized
                        allowed = verify_and_parse_scope()
                        if "analyze_net_logs" not in allowed:
                            write_receipt(event_id, "SWARM BLOCKED",
                                          "reason=out_of_scope action=analyze_net_logs")
                            log_audit("analyze_net_logs not in SCOPE.md — event blocked.", level="WARNING")
                            continue

                        host_metrics = get_windows_disk_metrics()

                        # --------------------------------------------------
                        # TIER 1: Qwen Network Triage
                        # --------------------------------------------------
                        triage_prompt = (
                            f"You are a network security triage agent.\n"
                            f"Analyze this network event log line:\n\n"
                            f"Event: '{raw_event}'\n\n"
                            f"{NET_SEVERITY_GUIDE}\n"
                            f"Return JSON ONLY. No explanation. No markdown.\n"
                            f'Template: {{"anomaly": true/false, "severity": "LOW"/"MEDIUM"/"HIGH", '
                            f'"event_type": "firewall_violation|port_scan|geo_anomaly|unusual_outbound|dns_anomaly|auth_storm|unknown", '
                        ...
                        try:
                            ...
                            event_type  = triage_data.get("event_type", "unknown")
                            ...
                            is_anomaly, severity, event_type, reason, confidence = (
                                True, "HIGH", "unknown", "Parsing failure — treated as anomaly.", 0
                            )

                        log_audit(
                            f"Tier 1 Net Triage -> "
                            f"Anomaly: {is_anomaly} | Type: {event_type} | "
                            f"Severity: {severity} | Confidence: {confidence}/100"
                        )

                        # --------------------------------------------------
                        # CONFIDENCE GATE → SWARM HUMAN HOLD
                        # --------------------------------------------------
                        if confidence < CONFIDENCE_THRESHOLD:
                            write_receipt(event_id, "SWARM HUMAN HOLD",
                                          f"confidence={confidence}/100 threshold={CONFIDENCE_THRESHOLD}")
                            log_audit(
                            ...
                            write_to_approval_boundary(
                                f"LOW CONFIDENCE NET EVENT — confidence={confidence}/100",
                            ...
                            print("\n" + "~" * 60)
                            print("~~~ LOW CONFIDENCE NETWORK ALERT ~~~")
                            print("~" * 60)
                        ...
                        # --------------------------------------------------
                        # TIER 2: Llama Network Remediation
                        # --------------------------------------------------
                        if is_anomaly:
                            log_audit("Escalating to Tier 2 Network Executive...")

                            if sys.platform == "win32":
                                ctypes.windll.user32.MessageBeep(0)

                            executive_prompt = (
                                f"You are a network security incident response specialist.\n"
                                f"A network security event requires a remediation ticket.\n\n"
                                f"Event:      {raw_event}\n"
                                f"Type:       {event_type}\n"
                                ...
                                f"Host:       {host_metrics}\n\n"
                                f"RULES:\n"
                                f"1. Windows environment only. Use Windows-native commands: "
                                f"netstat, netsh advfirewall, Get-NetFirewallRule, Get-NetTCPConnection, "
                                f"Get-NetAdapter, Test-NetConnection, nslookup, Resolve-DnsName.\n"
                                f"2. Do NOT use Linux tools (iptables, tcpdump, grep, nmap).\n"
                                f"3. Write a precise 3-point remediation ticket. "
                                f"Include the specific Windows command for each step.\n"
                                f"4. State what the operator should verify BEFORE taking any action.\n"
                                f"5. If the event could be a false positive, say so explicitly."
                            )

                            final_report = query_local_llm("llama3.2:3b", executive_prompt)
                            write_to_approval_boundary(
                                f"NET ALERT: {severity} {event_type} (confidence={confidence}/100)",
                                final_report
                            )
                            write_receipt(event_id, "SWARM DONE",
                                          f"anomaly={is_anomaly} type={event_type} severity={severity} confidence={confidence}/100")

                            print("\n" + "!" * 60)
                            print("!!! NETWORK SECURITY ALERT — HUMAN REVIEW REQUIRED !!!")
                            print("!" * 60)
                            print(f"[!] EVENT      : {raw_event}")
                            print(f"[!] TYPE       : {event_type}")
                            print(f"[!] REASON     : {reason}")
                            ...
                            write_receipt(event_id, "SWARM DONE",
                                          f"anomaly=False severity={severity} confidence={confidence}/100")
                            write_to_approval_boundary(
                                f"Net Nominal (confidence={confidence}/100)",
        ...
        log_audit("Network Agent stopped by operator.", level="WARNING")
        print("\n[-] Network Agent deactivated cleanly.")
